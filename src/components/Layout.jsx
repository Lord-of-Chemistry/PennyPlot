...
function Layout() {
  ...
  const [currency, setCurrency] = useState(() => {
    return (
      localStorage.getItem("pennyplot-currency") ||
      "NGN"
    );
  });
      ...
      setIsOnline((previous) => {
        if (!previous && showNotification) {
          ...
          const notification = createNotification({
            ...
            title: "Back online",
            message:
              "PennyPlot is connected to the internet again.",
          });
  ...
  */
  ...
  const budgets = useMemo(() => {
    try {
      ...
      console.error(
        "Failed to load budgets for alerts:",
        error,
      );
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error(
        "Failed to load budget alert history:",
        error,
      );
    }
  ...
  */
  ...
  return (
    <div className="min-h-screen bg-[#0f1714]">
      ...
      <main
        className={`bg-[#0f1714] text-white transition-[margin] duration-300 ease-in-out ${
          isCollapsed
            ? "md:ml-20"
            : "md:ml-56"
        }`}
      >
        ...
        <header className="sticky top-0 z-[150] border-b border-white/[0.06] bg-[#0f1714]/95 px-4 py-3 backdrop-blur-md">
        ...
        <div
          className={`fixed right-4 top-4 z-[300] flex items-center gap-2 rounded-xl border border-[#049552]/30 bg-[#1b2922] px-4 py-3 text-sm text-white shadow-2xl transition-all duration-300 ${
            showBackOnline
